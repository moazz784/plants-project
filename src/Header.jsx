      ...
      reader.onloadend = async () => {
        try {
          ...
          await api.users.updateMe({ 
            name: user.name, 
            profileImageBase64: base64Clean 
          });
          await refreshUser(); 
            ...
            {user ? (
              <>
                ...
                <div ref={dropdownRef} className="relative">
                  ...
                  <AnimatePresence>
                    {openProfileMenu && (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`absolute top-full mt-2 w-56 bg-white shadow-xl rounded-[10px] border border-gray-100 z-50 overflow-hidden ${i18n.language === 'ar' ? 'left-0' : 'right-0'}`}>
              ...
              <button onClick={() => navigate('/login')} className="bg-green-700 text-white px-5 py-2 rounded-full hover:bg-green-800 transition">{t('login')}</button>
            )}
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="lg:hidden p-1 text-green-800 active:scale-90"><svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">{isMenuOpen ? <path strokeWidth="2" d="M6 18L18 6M6 6l12 12" /> : <path strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7" />}</svg></button>
          </div>
        </div>
